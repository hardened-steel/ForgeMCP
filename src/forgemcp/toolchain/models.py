...
class CMakeKit(ForgeModel):
    ...
    origin: str = Field(default="standalone", min_length=1, description="Safe provider marker: standalone, visual_studio, explicit, or path; never a path.")
    compiler_family: str = Field(min_length=1, description="msvc, clang-cl, clang, gcc, or unknown.")
    driver_mode: str = Field(default="unknown", min_length=1, description="Compiler driver contract: cl, clang-cl, clang++, g++, or unknown.")
    abi: str = Field(default="unknown", min_length=1, description="Qualified ABI contract: msvc, llvm, gnu, or unknown.")

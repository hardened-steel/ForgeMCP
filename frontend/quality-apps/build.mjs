...
const targetFor = (name) => join(repositoryRoot, "src", "forgemcp", "apps", "assets", `quality-${name}.html`);
const assetSources = {
  overview: { script: "quality-overview-app.js", css: "quality-overview-app.css" },
  findings: { script: "quality-findings-app.js", css: "quality-findings-app.css" },
};
...
const [templateSource, themeSource, helperSource, runtimeSource] = await Promise.all([
...
for (const [name, sources] of Object.entries(assetSources)) {
  const [appSource, cssSource] = await Promise.all([
    readFile(join(sourceDirectory, sources.script), "utf8"),
    readFile(join(sourceDirectory, sources.css), "utf8"),
  ]);
  ...
  if (mode === "check") {
    const existing = await readFile(targetFor(name), "utf8");
    if (existing !== html) throw new Error(`quality-${name}.html is stale; run node frontend/quality-apps/build.mjs --write`);
  } else await writeFile(targetFor(name), html, "utf8");
}

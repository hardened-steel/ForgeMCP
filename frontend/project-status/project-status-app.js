...
function updateSelection() {
  ...
  root.querySelectorAll(".forge-project-component").forEach((button) => {
    button.setAttribute("aria-pressed", String(components[Number(button.dataset.index)] === state.selected));
  });
}
function select(component) { state.selected = component || null; state.preview = null; updateSelection(); renderDetail(); }
...
function renderComponents() {
  ...
  for (const [componentIndex, component] of state.status.components.entries()) {
    const button = el("button", "forge-project-component"); button.type = "button"; button.dataset.index = String(componentIndex); button.setAttribute("aria-pressed", String(component === state.selected)); button.setAttribute("aria-label", `${component.name}, ${component.state}. ${detailMessage(component)}`); button.append(el("span", `forge-status-symbol ${component.state}`, SYMBOLS[component.state]), el("span", "forge-component-name", component.name), el("span", "forge-component-state", component.state));
    button.addEventListener("click", () => select(component)); button.addEventListener("mouseenter", () => preview(component)); button.addEventListener("mouseleave", () => { state.preview = null; renderDetail(); }); button.addEventListener("focus", () => preview(component)); button.addEventListener("blur", () => { state.preview = null; renderDetail(); }); button.addEventListener("keydown", (event) => { if (event.key === "ArrowLeft" || event.key === "ArrowUp" || event.key === "ArrowRight" || event.key === "ArrowDown") { event.preventDefault(); const buttons = [...target.querySelectorAll("button")]; const index = buttons.indexOf(button); const columns = window.matchMedia("(max-width: 520px)").matches ? 2 : 3; const next = event.key === "ArrowLeft" ? index - 1 : event.key === "ArrowRight" ? index + 1 : event.key === "ArrowUp" ? index - columns : index + columns; (buttons[next] || button).focus(); } }); target.append(button);
  }
}
function render() {
...
connectMcpApp({ name: "forgemcp-project-status", ontoolinput: () => {}, ontoolresult: (result) => { state.status = validStatus(result); state.selected = state.status?.components[0] || null; state.preview = null; render(); }, onteardown: () => { state.tornDown = true; } }).catch(() => { if (!state.tornDown) render(); });
render();

...
| MCP Apps (72 tools, 18 resources) | SDK stdio Apps/no-Apps inventory audit, static UI source/asset checks, asset-freshness build, and wheel smoke | The Apps client sees exactly one nested App binding for every public tool, all 18 exact-MIME `ui://` resources, empty CSP lists, no permissions, and unchanged representative results. The non-Apps client keeps the identical tool schemas, annotations, structured content, and textual fallback. Every view is read-only with no host tool calls; visual rendering is reviewed manually in MCP Inspector. |

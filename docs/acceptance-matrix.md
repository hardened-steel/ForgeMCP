...
| MCP Apps status views | SDK stdio Apps/no-Apps, static UI source checks, wheel smoke, and Inspector v2 | The Apps client sees the stable extension, nested `git__status` and `project__status` metadata, exact `ui://forgemcp/git/status` and `ui://forgemcp/project/status` MIME, empty CSP lists, no permissions, and normal tool results. The non-Apps client keeps the same plain tool/structured fallback. Both views are read-only with no host tool calls; manual rendering review uses MCP Inspector. |
